...
exports.skillInTraining = function(req, resp) {
 var charID = req.params.charId;
 eveapi.fetch('char:SkillInTraining', { characterID: charID }, function(err, skillData) {
  console.log(skillData);
  resp.send(skillData);
 });
}

exports.getMails = function(req, resp) {
  var charID = req.params.charId;
  eveapi.fetch('char:MailMessages', { characterID: charID }, function(err, mails) {
      async.map(Object.keys(mails.messages), 
        function(messageID, messageCallback) {
          var message = mails.messages[messageID];
          getCharacterName(message.senderID, function(err, charName) {
             var data = {
...
function getCharacterName(characterID, callback) {
  eveapi.fetch('eve:CharacterName', { ids: characterID }, function(err, charName) {
    if (err) callback(err, null);
    var name = charName.characters[characterID].name;
    callback(null, name);
  });
}