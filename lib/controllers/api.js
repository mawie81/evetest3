...
function getCharacterSheet(characterID, callback) {
  eveapi.fetch('char:CharacterSheet', { characterID: characterID }, function(err, sheet) {
      ...
      for(var skillId in sheet.skills) {
        totalSkillpoints += sheet.skills[skillId].skillpoints;
      }

      console.log('Char ' + characterID + ' points: ' + totalSkillpoints);
...
function getTypename(typeID, callback) {
  db.get('select typeName from invTypes where typeID = ?', typeID, function(err, row) {
      if (err) callback(err, null);
      callback(null, row.typeName);
    });
}
