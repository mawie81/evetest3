    ...
    async.map(Object.keys(transactions.transactions), 
        function(transactionID, transactionCallback) {
          ...
          var data = {
            ...
            transationType: transaction.transactionType,
            transactionFor: transaction.transactionFor,
            price: transaction.transactionType == 'buy' ? transaction.price * -1 : transaction.price
          };
...
function cacheTypenames() {
  db.each('select typeID, typeName from invTypes', function(err, row) {
    typeCache[row.typeID] = { typeName: row.typeName };
  });
}
